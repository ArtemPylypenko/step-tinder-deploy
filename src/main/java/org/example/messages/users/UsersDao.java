...
public class UsersDao implements Dao<org.example.users.User> {
    Connection conn;
    ...
    @Override
    public org.example.users.User get(String id) throws SQLException {
        Optional<org.example.users.User> userOptional = Optional.empty();
        ...
        return new org.example.users.User(id, login, password, name, imgURL);

    }

    public List<org.example.users.User> getAll() throws SQLException {
        PreparedStatement statement = conn.prepareStatement("select * from users");
        ResultSet res = statement.executeQuery();
        List<org.example.users.User> userList = new ArrayList<>();
        org.example.users.User tmp = null;
        while (res.next()) {
            ...
            tmp = new org.example.users.User(id, login, password, name, imgURL);
            userList.add(tmp);
        }
        return userList;
    }

    public void save(org.example.users.User t) throws SQLException {
        try {
    ...
    public org.example.users.User getUserByLogin(String login) throws SQLException {
        PreparedStatement statement = conn.prepareStatement("select * from users where login = ?");
        statement.setString(1, login);
        ResultSet res = statement.executeQuery();
        org.example.users.User u = null;
        while (res.next()) {
            String id = res.getString("id");
